...
int main()
{
	...
	while (1)
	{
		...
		if (ret == ENTER)
		{
			switch (index)
			{
			case 0:system("二次函数.exe"); break;		//二次函数
			case 1:system("一元三次方程.exe"); break;	//一元三次方程
			case 2:system("基本不等式.exe"); break;		//基本不等式
			case 3:heart(); break;						//心形点阵
			case 4:system("进制转换.exe"); break;		//进制转换
			case 5:system("质数.exe"); break;			//质数
			case 6:system("勾股数组.exe"); break;		//勾股数组
			case 7:return 0;							//退出
			}
		}
	}
